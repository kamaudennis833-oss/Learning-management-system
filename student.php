...
if (isset($_POST['update_profile'])) {
    ...
    /*
     * NOTE: Token rotation removed here.
     * Rotating the CSRF token on every successful action invalidates the
     * token already embedded in any other already-loaded copy of this page
     * (e.g. a second tab, or this same page if the user submits another
     * modal before reloading). That was the root cause of the
     * "Invalid CSRF token" error. One token per session is sufficient here.
     */
...
if (isset($_POST['submit_payment'])) {
    ...
    if ($pay_course) {
        ...
        $_SESSION['flash']          = "Payment successful. Enrollment activated.";
        $_SESSION['active_section'] = 'payments';
        header("Location: student.php");
        exit;
    } else {
...
$progress  = (int)round((float)($data['avg_progress'] ?? 0));
...
$stmt = $conn->prepare("
    SELECT c.title, e.progress FROM courses c
...
$stmt = $conn->prepare("
    ...
    INNER JOIN courses c ON e.course_id=c.id
    WHERE e.user_id=? ORDER BY e.progress DESC
");
$stmt->bind_param("i", $user_id); $stmt->execute();
$courseProgress = $stmt->get_result(); $stmt->close();
...
/*
 * Active section — one-shot signal for where the page should land after a
 * server-side action (profile update, attendance, payment).
 * Read once and unset immediately: a plain reload of student.php with no
 * pending action will NOT have this set, so it correctly falls back to the
 * Dashboard. Only a redirect from one of the POST handlers above sets it.
 */
$active_section_map = [
...
/*
 * The Browse Courses search form submits via GET (so the search term is
 * shareable/bookmarkable), not POST — so it can't set the session flag the
 * way the other actions do. Detect it directly: a search submit always
 * includes ?search=... (even if empty) because of the hidden "section"
 * field. This does NOT make plain reloads sticky, because a plain reload
 * of student.php (no query string) won't have this param at all.
 */
...
/* =====
   SECTION — hidden by default, shown via JS
===== */
...
<div class="topbar">
    ...
    <h2>LMS Portal</h2>
    <span style="width:36px;"></span><!-- spacer -->
</div>
...
<div id="dashboardContent" class="section">
    ...
    <div class="two-col">
        <div class="card">
            ...
            <?php while ($row = $courses_widget->fetch_assoc()): ?>
...
<div id="coursesSection" class="section">
    ...
    <div class="card">
        <div class="card-title"><i class="fas fa-laptop-code"></i>Active Courses</div>
        <?php if ($active_courses_list->num_rows > 0): ?>
            <?php while ($course = $active_courses_list->fetch_assoc()): ?>
                <div class="course-row">
                    <div class="course-info">
                        <h4><?= htmlspecialchars($course['title'] ?? '') ?></h4>
                        <p><?= (int)($course['lessons'] ?? 0) ?> lessons • <?= (int)($course['progress'] ?? 0) ?>% done</p>
                        <div class="progress-bar-wrap"><div class="progress-fill" style="width:<?= (int)($course['progress'] ?? 0) ?>%"></div></div>
                    </div>
                    <a href="course1.php?id=<?= (int)$course['id'] ?>" class="btn small">Continue</a>
                </div>
            <?php endwhile; ?>
        <?php else: ?>
            <div class="activity-item"><p>No active courses</p><span>Browse courses to enroll</span></div>
        <?php endif; ?>
    </div>

    <div class="card">
        <div class="card-title"><i class="fas fa-check-double"></i>Completed Courses</div>
        <?php if ($completed_courses_list->num_rows > 0): ?>
            <?php while ($c = $completed_courses_list->fetch_assoc()): ?>
                <div class="activity-item">
                    <p><?= htmlspecialchars($c['title']) ?></p>
                    <span><?= !empty($c['completed_at']) ? 'Completed '.date("d F Y", strtotime($c['completed_at'])) : 'N/A' ?></span>
                </div>
            <?php endwhile; ?>
        <?php else: ?>
            <div class="activity-item"><p>No completed courses yet</p></div>
        <?php endif; ?>
    </div>

    <div class="card">
        <div class="card-title"><i class="fas fa-tasks"></i>Assignments</div>
        <?php if ($assignments->num_rows > 0): ?>
            <?php while ($a = $assignments->fetch_assoc()): ?>
                <div class="activity-item">
                    <p><?= htmlspecialchars($a['title'] ?? '') ?></p>
                    <span><?= htmlspecialchars($a['status'] ?? '') ?><?= !empty($a['due_date']) ? ' • Due '.date("d F Y", strtotime($a['due_date'])) : '' ?></span>
                </div>
            <?php endwhile; ?>
        <?php else: ?>
            <div class="activity-item"><p>No assignments</p></div>
        <?php endif; ?>
    </div>
...
<div id="progressSection" class="section">
    ...
    <div class="card">
        ...
        <?php if ($courseProgress->num_rows > 0): ?>
            <?php while ($cp = $courseProgress->fetch_assoc()): ?>
                <div class="course-row">
                    <div class="course-info">
    ...
    <div class="card">
        ...
        <?php endif; ?>
        <a href="take_quiz.php" class="btn small" style="margin-top:10px;"><i class="fas fa-question-circle"></i> Go to Quizzes</a>
    </div>
            ...
            <?php foreach ($browseCoursesData as $course): ?>
                <div class="course-row">
                    <div class="course-info">
                        ...
                        <p style="color:var(--gray-500);font-size:0.82rem;"><?= htmlspecialchars($course['instructor']) ?> • <?= (int)$course['lessons'] ?> lessons • <?= number_format($course['enrolled_students']) ?> students</p>
                    </div>
                    <a href="enroll.php?course_id=<?= (int)$course['id'] ?>" class="btn small">Enroll</a>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
...
<div id="settingsSection" class="section">
    <div class="card">
        ...
        <div class="activity-item"><p>Notifications</p><span>Email: <?= $settingsRow['email_notifications'] ? 'On ✅' : 'Off ❌' ?> &nbsp;|&nbsp; SMS: <?= $settingsRow['sms_notifications'] ? 'On ✅' : 'Off ❌' ?></span></div>
        ...
        <form method="POST" style="display:flex;flex-direction:column;gap:0;">
            ...
            <div class="form-group">
                ...
                <select name="course_id" required>
                    ...
                    <?php
                    $pCourses = $conn->query("SELECT id, title, price FROM courses WHERE course_type='Paid'");
                    while ($c = $pCourses->fetch_assoc()):
                    ?>
                        <option value="<?= (int)$c['id'] ?>"><?= htmlspecialchars($c['title']) ?> (KES <?= number_format($c['price']) ?>)</option>
                    <?php endwhile; ?>
                </select>
            </div>
            <div class="form-group">
            ...
            <button type="submit" name="submit_payment" class="btn" style="width:100%;justify-content:center;"><i class="fas fa-lock"></i> Pay &amp; Enroll</button>
        </form>
    </div>
</div>
...
/*
 * ACTIVE_SECTION — sent once by PHP after a redirect from a POST handler
 * (profile update, attendance, payment). Empty string means
 * "no pending action" — i.e. this is a plain page load/reload, so we land
 * on the Dashboard. This value is NOT derived from the URL, so refreshing
 * the page (which keeps the same URL) will not re-trigger it.
 */
const ACTIVE_SECTION = <?= $active_section_js ?>;

/* On page load — choose which section to show */
window.addEventListener('load', function () {
    const targetId = ACTIVE_SECTION || 'dashboardContent';
    const target   = document.getElementById(targetId);
    if (target) target.style.display = 'block';

    /* Sync the sidebar's active nav highlight to match */
    document.querySelectorAll('#navMenu li').forEach(li => li.classList.remove('active'));
