...
if (isset($_POST['submit_ticket'])) {
    ...
    /* =========================
       CSRF CHECK
       (student.php's ticket form already sends this field —
        this handler was silently ignoring it before.)
    ========================= */
    if (
    ...
    /* =========================
       INPUT SANITIZATION
       (trim/strip only — actual SQL safety now comes from
        prepared statements below, not string escaping)
    ========================= */
    ...
    /* =========================
       INSERT TICKET (prepared statement)
    ========================= */
    $stmt = $conn->prepare("
...
/* If this script is reached without submit_ticket set, there's nothing to do here. */
header("Location: student.php");
exit();